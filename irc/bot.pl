        ...
        if ( ( $message =~ /^cinci_pm_bot/ ) || ( $message =~ /^bot\s+/ ) )
        {
            ...
            elsif ( $message =~ /^weather/ )
            {
                ...
                if ( $message =~ /inside/ )
                {
                    my $dt = DateTime->now;
                    my $tmp =
                      `/Users/mslagle/Applications/HardwareMonitor.app/Contents/MacOS/hwmonitor -f 2>/dev/null| grep AMBIENT | cut -d':' -f2 | cut -d' ' -f2`;
                    my $tmpc =
                      `/Users/mslagle/Applications/HardwareMonitor.app/Contents/MacOS/hwmonitor 2>/dev/null| grep AMBIENT | cut -d':' -f2 | cut -d' ' -f2`;
                    $tmp =~ s/[\r\n]//g;
                    $tmpc =~ s/[\r\n]//g;
                    my $metar;
                    if ($seesmetar)
                    {
                        $metar =
                            '(KCZ0 '
                          . sprintf( '%02s%02s%02s', $dt->day, $dt->hour, $dt->minute )
                          . 'Z VRB001KT 10SM OVC000CEL '
                          . sprintf( '%02s', $tmpc )
                          . '/00 AXXXX RMK A01)';
                    }
                ...
                else
                {
                    ...
                    print $sock
                      "PRIVMSG $lchan :$w->{conditionstext} $w->{temperature_f} degrees - $w->{pressure_inhg} in $metar\r\n";
            ...
            elsif ( $message =~ /^forecast/ )
            {
                eval {
                    ...
                    print $sock "PRIVMSG $lchan :"
                      . pack( "A12 A8 A8 A8 A8", $r1[0], $r1[1], $r1[2], $r1[3], $r1[4] ) . "\r\n";
                    print $sock "PRIVMSG $lchan :"
                      . pack( "A12 A7 A7 A7 A7", $r2[0], $r2[1], $r2[2], $r2[3], ' ' . $r2[4] ) . "\r\n";
                    print $sock "PRIVMSG $lchan :"
                      . pack( "A12 A8 A8 A8 A8", $r3[0], $r3[1], $r3[2], $r3[3], $r3[4] ) . "\r\n";
                };
            ...
            elsif ( $message =~ /^lastseen/ )
            {
                ...
                if ($user)
                {
                    eval {
                        my $dbh = &get_dbh;
                        my $userref =
                          $dbh->selectrow_hashref( "select * from irclogs.irc_user where shortname=?", undef, $user );
                        my $lastseen = $dbh->selectrow_hashref(
                            "select il.logged_at,ic.name from irclogs.irc_log il 
                        ...
                        {
                            print $sock
                              "PRIVMSG $lchan :last saw $user at $lastseen->{logged_at} on $lastseen->{name}\r\n";
                        }
                        else
                        {
                ...
                if ($user)
                {
                    eval {
                        ...
                        my $userref =
                          $dbh->selectrow_hashref( "select * from irclogs.irc_user where shortname=?", undef, $user );
                        my $chanref =
                          $dbh->selectrow_hashref( "select * from irclogs.irc_channel where name=?", undef, $lchan );
                        my $lastsaid = $dbh->selectrow_hashref(
                            "select * from irclogs.irc_log il 
                        ...
                        {
                            print $sock
                              "PRIVMSG $lchan :\caACTION has never seen $user say anything on $chanref->{name} before\ca\n";
                        }
                    };
                    if ($@)
                    {
